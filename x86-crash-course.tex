...
\begin{frame}{Syntax: AT\&T vs. Intel}
  \begin{center}
    \emph{move the value 0 to \texttt{EAX}}
    \par\vskip 0.8em\begin{tabular}{ccc}
      \textbf{AT\&T} & $\qquad$ & \textbf{Intel} \\
      \texttt{mov \$0x0,\%eax} & & \texttt{mov eax, 0h}
    \end{tabular}
  \end{center}
...
\begin{frame}
  \frametitle{Syntax: AT\&T vs. Intel (pointer dereferencing)}
  \begin{center}
    \emph{move the value 0 to the address contained in \texttt{EBX}+4}
    \par\vskip 0.8em\begin{tabular}{ccc}
      \textbf{AT\&T} & $\qquad$ & \textbf{Intel} \\
      \texttt{mov \$0x0,0x4(\%ebx)} & & \texttt{mov [ebx+4h],0h}
    \end{tabular}
  \end{center}
    \begin{block}{Main differences}
    \begin{itemize}
...
\end{lstlisting}
      ...
      \begin{center}
        ...
        \begin{center}
          \begin{tikzpicture}[every
            node/.style={font=\ttfamily\footnotesize}]
            \node [stack,rectangle split parts=6] (s) {%
              ...%
              \nodepart{two}%
              2 (loc.)%
              \nodepart{three}%
