...
\author[NECSTLab]{Mario Polino, DEIB}
...
\begin{frame}
  \frametitle{(10) Load Effective Address LEA}
  \begin{itemize}
  \item{LEA \underline{\textbf{destination}}, \underline{\textbf{source}}}\\
    \textbf{source} a memory location\\
    \textbf{destination} register\\


  \item{Like a MOV, but it is storing the pointer not the value}
  \item{Example: LEA eax, [ebx+8]} 
  \end{itemize}

    \begin{columns}
      \begin{column}{0.4\columnwidth}
          Put [ebx+8] into EAX. \textbf{EAX = 0x00403A48}. \\
          \textbf{N.B.} \\
          with MOV eax, [ebx+8] \\
           EAX = 0x0012C140
      \end{column}
      \begin{column}{0.6\columnwidth}
      \begin{figure}
        \includegraphics[width=1\textwidth]{images/lea.png}

        \label{Lea memory status}
      \end{figure}
      \end{column}
    \end{columns}
...
\begin{frame}
  \frametitle{(12) Basic Instruction INT}
  \begin{itemize}
...
\begin{frame}
  \frametitle{(13) Syscall/SYSENTER}
