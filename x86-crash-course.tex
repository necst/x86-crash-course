...
\begin{itemize}
	...
	\item Evolved in a 32-bit ISA (1985, Intel 80386) and a 64-bit ISA (2003, AMD Opteron)
	\item CISC design (e.g.,~string operations)
	\item Many ``legacy'' features (e..g,~segmentation)
	\item We'll see the ``basic'' ISA --- there are various processor-specific features and extensions, such as SIMD (MMX, SSE, \dots) with their own instructions and registers\footnote{Complete reference: Intel Software Developer's Manual, about 5,000 pages (\url{https://software.intel.com/en-us/articles/intel-sdm})}
\end{itemize}
\end{frame}
...
\begin{frame}
 ...
 Assembly language: specific to each ISA, mapped to binary code
 	...
 	\begin{figure}
  	...
  	\end{figure}
For simplicitly, we don't deal with the \emph{linking} process.
 \end{frame}
